...
import FavouritesPage from '../pages/favourite-movies';

const mockRouter = { isFallback: false, asPath: '/favourite-movies' };
...
const sampleData = [
  ['Name', 'Notes'],
  ['The Matrix', 'Genre-defining sci-fi'],
...
describe('FavouritesPage (favourite-movies)', () => {
  beforeEach(() => {
  ...
  it('renders the page header with the favourite movies title', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('post-header')).toHaveTextContent('Favourite Movies');
  });

  it('passes the sheet data and cover art through to FavouriteResults', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('The Matrix');
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Heat');
  });

  it('renders a share bar and the favourites hub link', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('share-bar')).toHaveTextContent('Favourite Movies');
