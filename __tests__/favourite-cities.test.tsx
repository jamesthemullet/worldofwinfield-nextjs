...
import FavouritesPage from '../pages/favourite-cities';

const mockRouter = { isFallback: false, asPath: '/favourite-cities' };
...
const sampleData = [
  ['Name', 'Notes'],
  ['Tokyo', 'Neon-lit and endless'],
  ['Lisbon', 'Sunny and hilly'],
];
...
describe('FavouritesPage (favourite-cities)', () => {
  beforeEach(() => {
  ...
  it('renders the page header with the favourite cities title', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('post-header')).toHaveTextContent('Favourite Cities Visited');
  });

  it('passes the sheet data and cover art through to FavouriteResults', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Tokyo');
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Lisbon');
  });

  it('renders a share bar and the favourites hub link', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('share-bar')).toHaveTextContent('Favourite Cities Visited');
