...
import FavouritesPage from '../pages/favourite-djs';

const mockRouter = { isFallback: false, asPath: '/favourite-djs' };
...
const sampleData = [
  ...
  ['Peggy Gou', 'Consistently great sets'],
];
...
describe('FavouritesPage (favourite-djs)', () => {
  beforeEach(() => {
  ...
  it('renders the page header with the favourite DJs title', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('post-header')).toHaveTextContent('Favourite DJs');
  });

  it('renders the cover art disclaimer', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(
      screen.getByText(/Photos are matched automatically against Discogs/),
    ).toBeInTheDocument();
  });

  it('passes the sheet data and cover art through to FavouriteResults', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Carl Cox');
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Peggy Gou');
  });

  it('renders a share bar and the favourites hub link', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('share-bar')).toHaveTextContent('Favourite DJs');
