...
import FavouritesPage from '../pages/favourite-countries';

const mockRouter = { isFallback: false, asPath: '/favourite-countries' };
...
describe('FavouritesPage (favourite-countries)', () => {
  beforeEach(() => {
  ...
  it('renders the page header with the favourite countries title', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('post-header')).toHaveTextContent('Favourite Countries Visited');
  });

  it('passes the sheet data and cover art through to FavouriteResults', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Japan');
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Portugal');
  });

  it('renders a share bar and the favourites hub link', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('share-bar')).toHaveTextContent('Favourite Countries Visited');
