...
import FavouritesPage from '../pages/favourite-restaurants';

const mockRouter = { isFallback: false, asPath: '/favourite-restaurants' };
...
describe('FavouritesPage (favourite-restaurants)', () => {
  beforeEach(() => {
  ...
  it('renders the page header with the favourite restaurants title', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('post-header')).toHaveTextContent('Favourite Restaurants');
  });
  ...
  it('passes the sheet data and cover art through to FavouriteResults', () => {
    ...
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Franco Manca');
  });

  it('renders a share bar and the favourites hub link', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('share-bar')).toHaveTextContent('Favourite Restaurants');
