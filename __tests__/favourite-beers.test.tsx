...
import FavouritesPage from '../pages/favourite-beers';

const mockRouter = { isFallback: false, asPath: '/favourite-beers' };
...
describe('FavouritesPage (favourite-beers)', () => {
  beforeEach(() => {
  ...
  it('renders the page header with the favourite beer title', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('post-header')).toHaveTextContent('Favourite Beer');
  });

  it('passes the sheet data and cover art through to FavouriteResults', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('Shine');
    expect(screen.getByTestId('favourite-results')).toHaveTextContent('DIPA');
  });

  it('renders a share bar and the favourites hub link', () => {
    render(<FavouritesPage data={sampleData} coverArtByTitle={coverArtByTitle} />);
    expect(screen.getByTestId('share-bar')).toHaveTextContent('Favourite Beer');
